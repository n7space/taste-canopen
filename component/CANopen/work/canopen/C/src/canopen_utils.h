...
static co_unsigned8_t map_nmt_command_from_asn(asn1SccCANopen_NMT_Command cmd) {
  switch (cmd) {
  ...
  default:
    printf("[CANopen] Warning: Unknown ASN.1 NMT command: %d\n", cmd);
    return CO_NMT_CS_START;
  }
}
...
static co_unsigned8_t map_nmt_state_from_asn(asn1SccCANopen_NMT_State state) {
  switch (state) {
  ...
  default:
    printf("[CANopen] Warning: Unknown ASN.1 NMT state: %d\n", state);
    return CO_NMT_ST_BOOTUP;
  }
}
...
static asn1SccCANopen_NMT_HeartbeatState
map_nmt_ec_state_to_asn(co_nmt_ec_state_t state) {
  ...
  default:
    printf("[CANopen] Warning: Unknown NMT EC state: %d\n", state);
    return asn1SccCANopen_NMT_HeartbeatState_occurred;
  }
}

static const char *map_nmt_ec_state_to_string(co_nmt_ec_state_t state) {
  switch (state) {
  case CO_NMT_EC_OCCURRED:
    return "OCCURRED";
  case CO_NMT_EC_RESOLVED:
...
static asn1SccCANopen_NMT_HeartbeatReason
map_nmt_ec_reason_to_asn(co_nmt_ec_reason_t reason) {
  ...
  default:
    printf("[CANopen] Warning: Unknown NMT EC reason: %d\n", reason);
    return asn1SccCANopen_NMT_HeartbeatReason_timeout;
  }
}

static const char *map_nmt_ec_reason_to_string(co_nmt_ec_reason_t reason) {
  switch (reason) {
  case CO_NMT_EC_TIMEOUT:
    return "TIMEOUT";
  case CO_NMT_EC_STATE:
